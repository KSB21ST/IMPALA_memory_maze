...
def memory_maze_custom(**kwargs):
    return _memory_maze(5, 0, 80, max_rooms=1, room_min_size=5, **kwargs)
...
def _memory_maze(
    ...
    random_state=None,
    give_top_camera=False,
):

    walker = RollingBallWithFriction(camera_height=0.3, add_ears=top_camera)
    '''
    targets_per_room should be fixed, if you want to generate object!
    We are considering room without object, so targets_per_room = 0 (originally 5)
    '''
    ...
    task = MemoryMazeTask(
        ...
        wall_control=True,  # we want to control whether agent spawns close to wall or not
        close_wall=False,  # only works when wall_control=True, agent spawns close to wall
    )

    if top_camera or give_top_camera:
    ...
    if give_top_camera:
        obs_mapping = dict(obs_mapping, **{'top_camera': 'top_camera',})
